...
type GameCardRunModifierResult = GameCardRunDetails['modifierResults'][number]

export const gameCardRunModifierOutcomeStatuses = ['completed', 'failed', 'cancelled'] as const
export const gameCardRunPostRoundActions = ['continue', 'finish'] as const
...
export const gameCardRunSummaryFormSchema = z.object({
  ...
  postRoundAction: z.enum(gameCardRunPostRoundActions),
})

export type GameCardRunSummaryFormValues = z.infer<typeof gameCardRunSummaryFormSchema>
export type GameCardRunPostRoundAction = (typeof gameCardRunPostRoundActions)[number]
...
export interface ComputedGameCardRunModifierResolution {
      ...
      activeRun.modifierResults
        .map((modifier) => {
          ...
          return {
            ...
            countValue: deriveInitialCountValue(activeRun.baseScore, modifier, meta),
        ...
        .filter(
          (modifier): modifier is GameCardRunSummaryFormValues['modifiers'][number] =>
            modifier !== null,
        ),
    ),
  }
...
): ComputedGameCardRunModifierResolution {
...
function deriveSnapshotModifierRoundSummaryMeta(modifier: GameCardRunModifierResult) {
...
function deriveLegacyModifierRoundSummaryMeta(modifier: GameCardRunModifierResult) {
...
function deriveInitialCountValue(
  scoreUnit: number,
  modifier: GameCardRunModifierResult,
  meta:
...
function deriveInitialConditionMet(modifier: GameCardRunModifierResult) {
  const parsed = parseResolutionData(modifier.resolutionDataJson)
...
function normalizeOutcomeStatus(value: string | null | undefined) {
  ...
  return gameCardRunModifierOutcomeStatuses.includes(
    normalized as (typeof gameCardRunModifierOutcomeStatuses)[number],
  )
    ? (normalized as (typeof gameCardRunModifierOutcomeStatuses)[number])
    : null
}
...
function groupModifierSummaryEntries(entries: GameCardRunSummaryFormValues['modifiers']) {
  const grouped = new Map<string, GameCardRunSummaryFormValues['modifiers'][number]>()
...
function pickMergedOutcomeStatus(
  left: GameCardRunSummaryFormValues['modifiers'][number]['outcomeStatus'],
  right: GameCardRunSummaryFormValues['modifiers'][number]['outcomeStatus'],
) {
  const rank = {
    completed: 3,
    failed: 2,
