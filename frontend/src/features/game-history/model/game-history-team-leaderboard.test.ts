...
import { buildGameTeamLeaderboard, getRoundBonusDelta } from './game-history-team-leaderboard.ts'
...
describe('buildGameTeamLeaderboard', () => {
  it('ranks teams by their best round result instead of total score', () => {
    ...
    expect(leaderboard[0]?.latestRound.roundId).toBe('round-a-2')
    expect(leaderboard[0]?.rounds.map((round) => round.roundId)).toEqual(['round-a-2', 'round-a-1'])
...
function createRound(
  overrides: Partial<Parameters<typeof buildGameTeamLeaderboard>[0][number]> = {},
) {
  return {
    roundId: 'round-1',
    teamId: 'team-1',
