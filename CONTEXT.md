...
- game modifiers (phase 1): каталог модификаторов хранится в БД (`modifier_definitions`) с полями для расчёта (`mechanicType`, `effect` в `MetadataJson`, `scoringType`, `tier`, `defaultLimitPerGame`) и редактируемыми конфликтами (`modifier_conflicts`, `conflictingModifierIds` в create/update); первичный ключ модификатора — `Id` (Guid); форма каталога показывает русские категории механик (`rule_only`, `restriction_with_reward`, `kill_counter`, `multiplier`, `mentor`) и сохраняет структурированное влияние на будущий расчёт наград; в draft admin выбирает `enabledModifierIds` через `PUT /api/game/setup`; в active-игре `admin/moderator` может активировать модификатор `POST /api/game/modifiers/{modifierId}/activate`; состояние (`enabledModifierIds`, `activeModifiers`) входит в `GET /api/game`; realtime по `/hubs/game-board` событием `modifierActivated`;
