...
```text
bonus to one kill value = A × 5 × K
adjusted kill value     = B + A × 5 × K
total kills part        = (B + A × 5 × K) × K
                        = B × K + A × 5 × K²
```

То есть реализация `A × 5 × K²` — это дельта к общему результату, а пользовательская модель
сначала увеличивает стоимость одного убийства и затем умножает её на число убийств. Для карточки
`B = 100`, одной активации и трёх убийств бонус к стоимости равен `1 × 5 × 3 = 15`, новая
стоимость — `115`, итог за убийства — `115 × 3 = 345`. Для двух активаций новая стоимость равна
`130`, итог — `390`.
...
### J. Player-facing описание «Жажды»

- Статус: `Done 2026-08-24`.
- Проверки: полный backend suite `307/307 passed`; focused formula/contract/migration suite
  `25/25 passed`, включая реальный PostgreSQL; EF pending model changes — none; полный frontend
  gate `251/251 passed`, включая i18n, coverage, Knip и production build.
- Примечания: описание объясняет расчёт через накопленный бонус к стоимости одного убийства и
  пример `100 → 115; 115 × 3 = 345`. BehaviorV2-rule фиксирует тот же порядок операций. Миграция
  `20260824180435_ClarifyZhazhdaPlayerDescription` обновляет только стандартные старые тексты и
  сохраняет администраторские редакции; scoring formula и historical snapshots не меняются.
...
- Жажда увеличивает стоимость одного убийства на `5 × K` за каждую активацию; эквивалентная
  дельта к общему результату равна `5 × K²` на активацию;
