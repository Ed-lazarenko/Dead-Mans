...
- Новые игры читают полный pinned-набор редакций; legacy-игры используют только сохранённые
  activation/result snapshots и явно помечаются `legacy_unavailable`. Текущий каталог не
  используется для заполнения отсутствующих исторических данных.
