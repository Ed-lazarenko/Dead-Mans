...
- `question_definitions` — каталог вопросов. Soft-delete через `IsDeleted` /
  ...
  Принадлежит `question_vectors` (вектор создаётся автоматически при необходимости).
...
глобально включённых (`IsEnabled`), с включённым вектором, ещё не заданных в этой
игре. **Пустой выбор = вопросов нет** (ask-next вернёт `NoAvailableQuestions`) —
