...
- game modifiers (phase 1): глобальный каталог модификаторов, выбор `enabledModifierIds` в draft setup, активация `admin/moderator` только в `active`-игре через `modifierId`, хранение `activeModifiers` в БД и realtime событие `modifierActivated` на `game-board` hub
